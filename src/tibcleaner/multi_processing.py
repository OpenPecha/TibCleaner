...
def worker_task(args):
    ...
    if file.suffix.lower() in [".html", ".htm"]:
        html_to_txt_file(file, output_dir / file.suffix[1:])
    elif file.suffix.lower() == ".rtf":
        rtf_file_to_txt(file, output_dir / file.suffix[1:])
        ...
        convert_image_format_to_jpeg(file, output_dir / "img")
    elif file.suffix == ".TXT":
        convert_TXT_to_txt(file, output_dir / file.suffix[1:])
    else:
...
def convert_files_to_txt(
    all_files: List[Path], output_dir: Path, num_processes: int = 10
):
...
if __name__ == "__main__":
    ...
    num_processes = 10
    convert_files_to_txt(all_files, output_dir, num_processes)
